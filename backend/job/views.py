...
def updateJob(request, pk):
    ...
    if job.user != request.user:
        return Response({ 'message': 'You do not have authorization to update this job.' }, status=status.HTTP_403_FORBIDDEN)
...
def deleteJob(request, pk):
    ...
    if job.user != request.user:
        return Response({ 'message': 'You do not have authorization to delete this job.' }, status=status.HTTP_403_FORBIDDEN)
...
def applyToJob(request, pk):
    ...
    if job.lastDate < timezone.now():
        return Response({ 'error': 'You can not apply to this job. This position is no longer open.' }, status=status.HTTP_400_BAD_REQUEST)
